...
// Pagination: [Prev] [1] [2] [Next] */}
          ...
          {!onClose && (
            <div className="mt-4 overflow-y-auto max-h-[85%] rounded-lg">
              ...
              <div onClick={()=>handleToggleSearch()} className="flex  gap-2 items-center mb-3 transition-all ease-in-out duration-1000">  {/* search div*/}
                <FaSearch  size={25} />
                   <p className="text-[20px] font-bold font-oswald">Search</p>              
                   {
                    searchOnClose &&(
                       <input placeholder="Search here"  className={`rounded-[11px] h-[40px] border-1 transition-all duration-500 ease-in-out
      ${searchOnClose ? "w-40 opacity-100 px-2" : "w-0 opacity-0 px-0"}`}  border-1 "></input>
                    )
                   }
                   </div>
