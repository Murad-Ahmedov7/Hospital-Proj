...
<div className="w-full bg-white  p-4  mt-7 rounded-xl shadow-2xl shadow-blue-400 ">
  <table className="min-w-full divide-y h-[222px] text-center    divide-gray-200">
    <thead>
      <tr>
        <th>ID</th>
        <th>Name</th>
    ...
    <tbody>
      {allconsult.map(item => (
        <tr className='hover:bg-blue-50 transition-all duration-200 cursor-pointer' key={item.id}>
          <td>{item.id}</td>  
          <td>{item.name}</td>
          <td>{item.email}</td>
          <td>{item.date}</td>
