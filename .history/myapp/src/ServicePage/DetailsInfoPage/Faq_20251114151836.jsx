...
export default function Faq() {
  const handleButton = (id) => {
    ...
    useEffect(()=>{
          const data=faqData.filter((f)=>f.row.open===true)

    setOpenedElements(data)    const data=faqData.filter((f)=>f.row.open===true)

    setOpenedElements(data)
    })
  return (
              ...
              {item.row.map((qa, i) => (
                <div className="flex flex-col gap-4 " key={i}>
                  <div className="flex justify-between">
                    ...
                    <div onClick={() => handleButton(item.id)}>
                      <FaChevronDown />
                    </div>
                  </div>
