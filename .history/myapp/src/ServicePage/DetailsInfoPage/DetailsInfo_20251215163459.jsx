...
export default function DetailsInfo() {
  return (
    ...
    <div className="flex flex-col  rounded-3xl shadow-xl px-70 py-12 gap-12">
      {/* Hero şəkil */}
...
<div className="grid grid-cols-3 justify-center  gap-4">

  {
    ...
    detailsInfoCards.map((d,index)=>  index === 2 ? (
      <>
        <div></div> {/* Boş hüceyrə */}
        <div
          key={d.id}
          className="relative w-[290px] h-[320px] flex justify-center items-center group rounded-2xl overflow-hidden transition-transform duration-300 hover:-translate-y-2"
        >
          <d.icon size={70} />
          <p className="text-3xl font-semibold text-center text-white">{d.title}</p>
          <p className="w-[200px] text-white/70 text-center opacity-0 max-h-0 overflow-hidden group-hover:opacity-100 group-hover:max-h-32 transition-all duration-300">
            {d.desc}
          </p>
          <Link to={d.link} className="text-white opacity-0 group-hover:opacity-100 transition">
            {d.buttonText}
          </Link>
        </div>
      </>
