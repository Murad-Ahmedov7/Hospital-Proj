...
export default function Department() {
  ...
  <img src="/images/surgeryimg.jpg" alt="Not found" className="object-cover w-screen h-screen" />
  <div className="absolute inset-0 bg-[#3AA6B9]/90 z-0"></div>

  <div className="grid grid-cols-4 grid-rows-2 gap-10 mt-20 py-3 w-full h-full place-items-center absolute top-0 left-0 z-10">
    {departments.map((department) => (
      <Link
        key={department.id}
        to={department.Link}
        className="text-3xl border rounded-2xl hover:shadow-2xl font-bold text-white flex items-center justify-center flex-col gap-6 z-[99]"
      >
        <img className={`${department.width} ${department.height}`} src={department.img} alt={department.title} />
