...
export default function Cardiology() {
  return (
    <div className="flex flex-col items-center gap-12 min-h-screen w-screen bg-gray-50 pt-[80px] pb-10">
      
      {/* Main Section */}
      <div className="flex flex-col xl:flex-row gap-10 w-[92%]">

        {/* Left: Vitals & AI Risk */}
        <div className="flex-1 flex flex-col gap-8">
          
          {/* Greeting Card */}
          <div className="bg-white rounded-3xl shadow-2xl p-8 flex flex-col gap-6 transition-transform hover:scale-[1.02] duration-300">
            <div>
              <h1 className="text-3xl font-oswald font-bold text-gray-900">
                Hello, User 🖐
              </h1>
            ...
            {/* Current Vitals Header */}
            <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-100 to-cyan-200 text-cyan-800 font-oswald font-semibold shadow-md">
              <GiStethoscope size={28} />
              <span>Current Vitals</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mt-3">
              {vitalsData.map((vital) => (
                <div
                  key={vital.id}
                  className={`flex flex-col justify-between p-5 gap-2 rounded-2xl ${vital.bgColor} hover:scale-105 transition-transform duration-300 shadow-lg hover:shadow-2xl cursor-pointer group relative overflow-hidden`}
                >
                  {/* Icon Circle */}
                  <div className="absolute -top-5 -right-5 w-16 h-16 rounded-full bg-white/20 group-hover:bg-white/30 transition-all duration-500"></div>
                  <div className="flex items-center gap-3 z-10 relative">
                    <vital.Icon size={28} className="text-white" />
                    <p className="font-bold text-white font-oswald">{vital.title}</p>
                  </div>
                  <p className="text-white font-bold text-xl font-oswald z-10 relative">{vital.value}</p>
                </div>
              ))}
            </div>

            {/* AI Risk Card */}
            <div className="bg-gradient-to-r from-amber-100 to-amber-200 rounded-3xl p-6 shadow-md hover:shadow-xl transition-shadow duration-300">
              <p className="font-oswald font-bold mb-3 text-gray-800">
                AI Approximate Heart Disease Risk
              </p>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-full h-4 rounded-xl bg-amber-300/50 overflow-hidden">
                  <div className="w-[40%] h-full bg-amber-400 rounded-xl animate-pulse"></div>
                </div>
                <p className="font-oswald font-bold text-gray-800 w-[120px]">40% (Moderate)</p>
              </div>
              <p className="text-gray-700 mb-4 font-oswald text-sm">
              ...
              <button className="w-full md:w-[220px] bg-gradient-to-r from-blue-500 to-blue-600 text-white font-oswald font-semibold py-3 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300">
                Ask AI for Approximate Risk
              </button>
            </div>
        ...
        {/* Right: Medications & Quick Vitals */}
        <div className="flex flex-col gap-8 flex-1">

          {/* Medications Card */}
          <div className="bg-white shadow-2xl rounded-3xl p-6 flex flex-col gap-6 transition-transform hover:scale-[1.02] duration-300">
            <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-100 to-cyan-200 text-cyan-800 font-oswald font-semibold shadow-sm">
              <GiMedicinePills size={28} />
              <span>Current Medications</span>
            </div>
            <div className="flex flex-col gap-4 max-h-[450px] overflow-y-auto">
              {medicinesData.map((med) => (
                <div
                  key={med.id}
                  className={`flex justify-between items-center px-4 py-3 rounded-2xl shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300 bg-gray-50 border-l-4 ${med.borderColor} cursor-pointer relative overflow-hidden`}
                >
                  ...
                  <div className="absolute top-0 left-0 w-full h-full bg-white/0 group-hover:bg-white/5 transition-all duration-500 rounded-2xl"></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white shadow-2xl rounded-3xl p-6 flex flex-col gap-6 transition-transform hover:scale-[1.02] duration-300">
            ...
            <p className="text-gray-500 font-oswald">
              View your vitals at a glance and consult a doctor for detailed analysis.
            </p>
            <button className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white font-oswald font-semibold py-3 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300">
              Get Consult
            </button>
          </div>
