...
export default function PharmacyShop() {
  ...
  const [value, setValue] = useState([0, 1000]); // min və max dəyərlər

  
const MIN_DISTANCE = 100; // minimum məsafə 200 vahid

const handleValueChange = (newValue) => {
  const [newMin, newMax] = newValue;

  // sol Thumb sağ Thumb-dan MIN_DISTANCE-dən yaxın olarsa, dəyişmə
  if (newMax - newMin < MIN_DISTANCE) {
    return; // heç nə etmə, hərəkət bloklanır
  }

   if (newMax - newMin < MIN_DISTANCE) {
    if (newMin !== value[0]) {
      newMin = newMax - MIN_DISTANCE; // sol Thumb'u düzəlt
    }
    if (newMax !== value[1]) {
      newMax = newMin + MIN_DISTANCE; // sağ Thumb'u düzəlt
    }
  }
