...
export default function Pharmacy() {
  ...
  return (
    <div className="bg-gradient-to-bl from-blue-300 to-gray-100 w-screen h-screen flex   justify-center">
      <div className="w-screen h-[20%] relative  top-50 ">
        <div
          className="flex transition-transform duration-500 ease-out gap-20 w-screen  "
          style={{ transform: (curr === 0 || curr === 1 ) ? `translateX(-${curr * 50}%)` : setCurr(0) }}
        >
          {slides.map((s, index) => (
            <img
              key={index}
        ...
        <div className="absolute inset-0 flex items-center justify-between p-4">
          ...
          >
            <ChevronRight size={40} />
                ...
                <div className="absolute   top-[50%] bottom-0 left-0 right-0  -translate-x-1/2  -translate-y-1/2   ">
          <div className="flex items-center justify-center gap-2">
            {slides2.map((_, i) => (
              <div
                key={i}
                className={`transition-all w-3 h-3 bg-black/45 rounded-full ${
                  curr === i ? "p-12" : "bg-opacity-50"
                }`}
              >
