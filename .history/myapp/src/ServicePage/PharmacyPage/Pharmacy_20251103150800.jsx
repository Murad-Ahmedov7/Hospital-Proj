...
const [slides, setSlides] = useState([
  ...
  return (
    <div className="bg-gradient-to-bl w-screen min-h-screen flex justify-center items-center py-10">
      <div className="flex flex-col justify-center items-center w-full max-w-[1300px] gap-7">
        {/* Başlıqlar */}
        <p className="text-gray-600 text-2xl font-bold">Top Choices</p>
        <p className="font-bold text-5xl text-center">Our Most-liked Products</p>
        ...
        >
          {slides.map((s, i) => (
            <SwiperSlide key={i}>
              <div className="bg-gradient-to-bl from-gray-200/90 to-gray-400/10 rounded-xl w-full min-h-[400px] flex flex-col flex-shrink-0 px-5">
                
                {/* Şəkil */}
                <div className="flex justify-center items-center h-[68%]">
                  <img src={s.src} alt="Product" className="h-full object-contain" />
                </div>

                {/* Mətnlər və button */}
                <div className="bg-white rounded-2xl border border-white p-5 mt-3 flex flex-col gap-2 items-start hover:bg-[#5c45bf] transition-all duration-300">
                  <div className="flex justify-between w-full">
                    <p>Supplement</p>
                    <p>⭐4.8/5</p>
                  </div>
                  <p className="font-bold text-xl w-full">DermaGlow Skin Cream</p>
                  <div className="flex justify-between w-full items-center pt-3">
                    <button className="border-2 rounded-2xl px-4 py-2 font-semibold hover:bg-[#5c45bf] hover:text-white transition">
                      Add to cart
                    </button>
                    <p className="text-2xl text-[#5c45bf] font-bold">$42.00</p>
                  </div>
