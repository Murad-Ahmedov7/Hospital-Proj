  ...
  <div className="flex items-center gap-4">
    ...
    <div className="relative">
      ...
      {isOpen && (
        ...
        //   <div className="flex items-center gap-2 hover:bg-gray-200">  <FaBalanceScale  size={20} /> Muqayise</div>
    ...
    const content = item.label === "Basket" 
      ? <Link to="services/pharmacy/basket">{t(`navbar.myShopping.${item.label.toLowerCase()}`, { lng: currentLang })}</Link>
      : t(`navbar.myShopping.${item.label.toLowerCase()}`, { lng: currentLang });
      
    return (
      <div key={item.id} className="flex items-center gap-2 hover:bg-gray-200">
        <Icon size={20} /> {content}
      </div>
    );
  })}
      )}
