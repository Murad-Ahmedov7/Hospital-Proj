...
export default function Navbar() {
  ...
  return (
    <div
      className={`fixed top-0    w-[100%]  z-20 flex justify-between gap  items-center px-6 lg:px-4 xl:px-9    py-4 bg-white transition-all duration-700 ease-in-out ${
        firstScrollDone ? "border-2 border-cyan-500 rounded-3xl shadow-lg" : ""
      }`}
    >
            ...
            <div className="absolute top-full right-0 bg-white shadow-lg px-5 py-3 flex flex-col gap-2 z-30">
              {userMenuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon size={20} />{" "}
                    {t(`navbar.myShopping.${item.label.toLowerCase()}`, {
                      lng: currentLang,
                    })}
                  </div>
                );
