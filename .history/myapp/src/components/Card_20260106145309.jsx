...
const isPharmacyShopPage = ["/services/pharmacy/shop"].includes(location.pathname);
...
cconst Icon = isFavorite ? FaHeart : FaRegHeart;

 
 return (
    
<div className={` relative bg-gradient-to-bl from-gray-200/90 to-gray-400/10 px-5 flex-shrink-0 lg:w-80 xl:w-100 2xl:w-[450px]  min-h-130 rounded-xl shadow-lg group   ${isPharmacyShopPage ? `${styles.customW355} ${styles.customW500}` : ""} `}>
...
<div className=" absolute top-7 right-6 border border-white bg-white rounded-full   p-2">
  {isFavorite ? (<FaHeart color="red" onClick={handleFavorite} size={20} />):(<FaRegHeart  onClick={handleFavorite} size={20}/> ) }
</div>
