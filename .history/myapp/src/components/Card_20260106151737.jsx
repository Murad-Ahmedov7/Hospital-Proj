...
export default function Card({ s }) {
  ...
  return (
    <div className="relative bg-white rounded-xl shadow-lg group p-5">
      {/* Image + Heart */}
      <div className="flex justify-center items-center relative">
        <img
          src={s.src}
          alt={s.title}
          className="w-[80%] h-[320px] object-contain group-hover:-translate-y-3 transition-all"
        />
        <div className="absolute top-4 right-4 border border-white bg-white rounded-full p-2">
          <HeartIcon
            size={20}
      ...
      {/* Info */}
      <div className="w-full flex flex-col mt-3 gap-1 items-start bg-white rounded-2xl p-5 group-hover:bg-[#3AA6B9] transition-colors">
        <div className="flex justify-between w-full group-hover:text-white">
          <p>{s.category}</p>
          <p>${s.price}</p>
        </div>
