...
const { t, i18n } = useTranslation(); 
    ...
    return (
        ...
        <div className={`fixed top-0   w-[100%]  z-20 flex justify-between gap-11  items-center px-6 lg:px-4 xl:px-9    py-4 bg-white transition-all duration-700 ease-in-out ${firstScrollDone ? "border-2 border-cyan-500 rounded-3xl shadow-lg" : ""}`}>
  <Link to="/" className="flex-shrink-0 w-32 lg:w-30  ">
    <img src="/images/mediflow-logo.png" alt="Logo" className="w-full h-auto"/>
  </Link>

{/* overflow-x-hidden */}
  <div className="lg:flex gap-8  2xl:w-[490px]  justify-end xl:w-[500px] lg:w-[490px]    ">
    {links.map(link => (
  ...
  <div className="flex items-center gap-4">
    ...
    <select className="h-8" onChange={(e) => i18n.changeLanguage(e.target.value)}>
      <option value="en">En</option>
      <option value="az">Az</option>
    </select>
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full right-0 bg-white shadow-lg px-5 py-3 flex flex-col gap-2 z-30">
          <div className="flex items-center gap-2"><FaBalanceScale size={20}/> {t("navbar.myShopping.balance")}</div>
          <div className="flex items-center gap-2"><RiShoppingBasketLine size={20}/> {t("navbar.myShopping.basket")}</div>
          <div className="flex items-center gap-2"><FaRegHeart size={20}/> {t("navbar.myShopping.favorites")}</div>
        </div>
      )}
    </div>
  </div>
