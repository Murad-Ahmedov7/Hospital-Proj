...
useEffect(() => {
  if (i18n.language) {          // burada i18n.language artıq mövcuddursa
    if (!localStorage.getItem("language")) {
      localStorage.setItem("language", i18n.language);
    }
    console.log(localStorage.getItem("language")); // indi düzgün çıxacaq
  }
}, [i18n.language]);
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full right-0 bg-white shadow-lg px-5 py-3 flex flex-col gap-2 z-30">
          <div className="flex items-center gap-2"><FaBalanceScale size={20}/> {t("navbar.myShopping.balance",{ lng: currentLang })}</div>
          <div className="flex items-center gap-2"><RiShoppingBasketLine size={20}/> {t("navbar.myShopping.basket",{ lng: currentLang })}</div>
