...
//niye removu-isletdik???????
   ...
   <Listbox value={selected} onChange={setSelected}>
        <Listbox.Button className="p-2 border rounded bg-white w-full flex items-center gap-2">
          {selected.icon}
          {selected.label}
        </Listbox.Button>

        <Listbox.Options className="absolute bg-white shadow-lg rounded mt-2 w-full">
          {options.map((opt) => (
            <Listbox.Option
              key={opt.id}
              value={opt}
