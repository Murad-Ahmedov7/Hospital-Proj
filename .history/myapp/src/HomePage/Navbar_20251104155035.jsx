...
//   };
  ...
  if (location.pathname !== "/") {
    // Başqa route-dadırsa home-a yönləndir
    navigate("/", { replace: false });

    // Navigate tamamlandıqdan sonra scroll üçün timeout
    const scrollAfterNavigate = () => {
      const el = document.getElementById(id);
      if (el) {
        el.scrollIntoView({ behavior: "smooth", block: "start" });
      } else {
        // Hələ DOM render olunmayıbsa, bir az gecikmə ilə yenidən yoxla
        setTimeout(scrollAfterNavigate, 50);
      }
    };
    setTimeout(scrollAfterNavigate, 50);
  } else {
    // Artıq / path-dədirsə → birbaşa scroll
    const el = document.getElementById(id);
    el?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  window.history.replaceState(null, null, `#${id}`);
    ...
    return (
        ...
        <div className="flex gap-10 2xl:justify-between  lg:ml-3   justify-center font-oswald text-[#1F1534]   fixed   w-full pr-[8%] h-14 pt-6 rounded-[30px]  z-20   ">
